...
scores_bsc = np.arctanh(apply_mask(smooth_img(glob.glob('MaThe/avg_maps/model_{}_subj_*'.format(models[0])), fwhm=3.0), mask_img=mask))
scores_mfs = np.arctanh(apply_mask(smooth_img(glob.glob('MaThe/avg_maps/model_{}_subj_*'.format(models[1])), fwhm=3.0), mask_img=mask))
...
display.savefig('mean_diff_smoothed_model_{}.svg'.format('_'.join(models)))
display.savefig('mean_diff_smoothed_model_{}.png'.format('_'.join(models)))
fsf.save_map_avg('avg', mean_diff, threshold=None, model='diff_'+'_'.join(models))
display = fsf.plot_diff_avg(tscores, threshold)
display.savefig('ttest_diff_smoothed_model_{}.svg'.format('_'.join(models)))
display.savefig('ttest_diff_smoothed_model_{}.png'.format('_'.join(models)))
