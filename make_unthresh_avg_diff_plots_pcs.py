...
diff_scores = scores_bsc - (scores_mfs - scores_bsc)
...
display.savefig('mean_unthresh_diff_pc_model_{}.svg'.format('_'.join(models)))
display.savefig('mean_unthresh_diff_pc_model_{}.png'.format('_'.join(models)))
fsf.save_map_avg('avg', mean_diff, threshold=None, model='diff_pc_'+'_'.join(models))
display = fsf.plot_diff_avg(tscores, 0.001)
display.savefig('ttest_unthresh_diff_pc_model_{}.svg'.format('_'.join(models)))
display.savefig('ttest_unthresh_diff_pc_model_{}.png'.format('_'.join(models)))
