...
subj, split, models, fmri_type = [arg_namespace[key]
                                       for key in ['subj',
                                                   'split',
                                                   'models',
                                                   'fmri']]
...
for i, model in enumerate(models):
    lireg = LinearRegression().fit(stimuli[i], fmri_data)
    predictions = lireg.predict(stimuli[i])
...
dump(correlations,
     spenc_dir+'MaThe/scores/many_{}_lm_insample_subj_{}_split_{}.pkl'.format('_'.join(models),
                                                             subj, split),
     compress=3)
