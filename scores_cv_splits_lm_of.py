...
for train, test in cv:
    lireg = LinearRegression().fit(stimuli[train], fmri_data[train])
    predictions = lireg.predict(stimuli[test])
...
dump(test_score,
     spenc_dir+'MaThe/scores/{}_lm_of_subj_{}_split_{}.pkl'.format(model_name,
                                                             subj, split),
     compress=3)
