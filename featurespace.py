...
class Encoding(object):
    def __init__(self, fmri, predictions, scores, metric='r2'):
        self.fmri = fmri
        ...
        if metric in ('p', 'p_adj'):
            p_vals = metric_functions[metric](predictions, fmri)
            if metric == 'p':
    ...
    def get_predictions(self, threshold):
        return self.predictions[:, self.get_best_voxels(threshold)]

    def get_fmri(self, threshold):
        return self.fmri[:, self.get_best_voxels(threshold)]
...
metric_functions = {'r2' : r2_map_from_predictions,
                    'r' : r_map_from_predictions,
                    'p' : p_map_from_predictions,
                    'p_adj' : p_adj_map_from_predictions}
...
def load_data_new(subj, model='logBSC_H200', metric='r2'):
    '''Loads data and returns them as a namedtuple'''
    fmri_data = np.hstack(
            [joblib.load(('/home/data/scratch/mboos/prepro/'
                          'fmri_new_subj_{}_split_{}.pkl').format(subj, i))
