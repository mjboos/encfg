...
maps = [sorted(glob.glob('MaThe/maps/mni/model_{}_*whole_avg_map*'.format(model))) for model in ['lda', 'speaker', 'emotions']]
...
for subj, ft_maps in zip(sorted(valid_subjs), zip(*maps)):
    ...
    for ind_ft_map, color in zip(ft_maps, ['r', 'b', 'g']):
        level_thr = np.percentile(apply_mask('MaThe/maps/'+ind_ft_map.split('/')[-1], mask), 99.5)
        display.add_contours(ind_ft_map, colors=[color], levels=[level_thr], alpha=0.6, linewidths=3.0)
    display.savefig('contours_loose_subj_{}.png'.format(subj))
    plt.close()
