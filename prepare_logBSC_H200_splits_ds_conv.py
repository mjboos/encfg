...
patches = load(spenc_dir+'MaThe/'+\
                      'transformed_data/log_sparse_patches_H200_converged.pkl')
...
# average across 2s
patches = np.mean(np.reshape(patches, (patches.shape[0], 6, 10, 200)), axis=-2)
patches = np.reshape(patches, (-1, 1200))

dump(patches,
     spenc_dir+'MaThe/prepro/logBSC_H200_ds_conv_stimuli.pkl')
